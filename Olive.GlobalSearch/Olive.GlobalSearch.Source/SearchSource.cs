    ...
    public abstract class SearchSource
    {
        ...
        protected void Add(SearchResult result)
        {
            ...
            if (result.Url.StartsWith("~/"))
                result.Url = Context.Current.Request().GetAbsoluteUrl(result.Url.TrimStart("~"));

            if (result.IconUrl.OrEmpty().StartsWith("~/"))
                result.IconUrl = Context.Current.Request().GetAbsoluteUrl(result.IconUrl.TrimStart("~"));
